...
class BookForm(forms.ModelForm):
    ...
    class Meta:
        model = Book
        fields = ('genre', 'category', 'sku', 'ISBN',
        'author', 'title', 'link', 'pages', 'first_published',
        'this_publication_date', 'condition', 'description',
        'language', 'price', 'rating', 'image',)

    image = forms.ImageField(label='Image', required=False, widget=MyClearableFileInput)

    def __init__(self, *args, **kwargs):
        """
        Add placeholders and classes, set autofocus on first field. And adds genre items to dropdown
        """
